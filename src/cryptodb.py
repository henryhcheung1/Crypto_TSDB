...
class CryptoDB:
    ...
    def pull_data(self, symbol: str, start_time: str, end_time: str, interval: str, store_tsdb: bool=False):
        """
        ...
        if interval in meta.time_intervals:
            ...
            data = self.crypto_api.fetch_data_between(symbol=symbol, start_time=start_time, end_time=end_time, interval=interval)

        elif interval in meta.intraday_intervals:
            # fetch from intraday API
            data = self.crypto_api.fetch_intraday_data(symbol=symbol, start_time=start_time, end_time=end_time, interval=interval)
